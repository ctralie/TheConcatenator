...
\begin{equation}
    \label{eq:sparseklhgrad}
    \vec{h_i}^{\ell}[k]  \gets \vec{h_i}^{\ell-1} \left(  \frac{\sum_n (W_{n, \vec{s_i}[k]}) (\vec{v_t}[n]) / (\vec{\Lambda_i}^{\ell-1}[n]) }{ (\sum_{n} W_{n, \vec{s_i}[k]}) + \alpha[k] \vec{h_i}^{\ell-1}}[k] \right)
\end{equation}

Intuitively, if $s_i[k]$ is an activation that's very quiet, $\alpha[k]$ will be a 1, which will contribute an extra term to the demininator that shrinks $\vec{h_i}^{\ell-1}$ down (for a derivation of similar additive constraints on NMF, refer to \cite{virtanen2007monaural}).
