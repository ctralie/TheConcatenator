...
\subsection{Practical Probabilistic Implications}
\label{sec:practical}
...
\begin{figure}
    ...
    \caption{The geometric distribution causes the activation distribution of The Concatenator to be more exponential in nature than those of Driedger's technique.}
...
\begin{figure}
    ...
    \caption{The Concatenator preserves pitch reasonably often on MDB-stem-synth at all but the lowest octave.}
    \label{fig:PitchTests}
\end{figure}

\textbf{Reproducing Pitch.} In addition to fits and grain lengths, we seek to quantify how well The Concatenator reproduces target pitch.  Using the Woodwinds corpus, we use all of the stems in the MDB-stem-synth dataset \cite{salamon2017analysis} as targets, and we compare ground truth annotations to the pitches estimated with CREPE \cite{kim2018crepe} at different octaves on both the raw target and the synthesized audio for various $P$.  Even though CREPE was not trained on concatated audio, it reports pitch nearly as clearly as on the raw target for most octaves except for C1, which makes sense since the spectral resolution is only 21.5hz.  Furthermore, since the bassoon is the only instrument out of 10 in the Woodwinds dataset that has notes in the C2 octave, additional particles are needed for precise pitch in that octave.  This makes sense in light of Equation~\ref{eq:timeadjacentprobmodified}.
